...
def main():
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            cv2.putText(frame, f"GAZE: {gaze_text}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            if blink_text:
                cv2.putText(frame, blink_text, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

            frame_idx += 1
            if frame_idx % LOG_EVERY_N_FRAMES == 0:
                print(f"[NORMAL ] FPS: {fps:.1f}  GAZE: {gaze_text}  BLINK: {blink_text or '-'}")

            cv2.imshow("Mediapipe FPS Test (Normal)", frame)
